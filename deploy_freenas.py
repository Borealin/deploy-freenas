...
def deploy():
    parser = argparse.ArgumentParser(
        description='Import and activate a SSL/TLS certificate into FreeNAS.')
    ...
    ), help='Path to config file, defaults to deploy_config.')
    ...
    PRIVATEKEY_PATH = deploy.get(
        'privkey_path', "/root/.acme.sh/" + DOMAIN_NAME + "/" + DOMAIN_NAME + ".key")
    FULLCHAIN_PATH = deploy.get(
        'fullchain_path', "/root/.acme.sh/" + DOMAIN_NAME + "/fullchain.cer")
    PROTOCOL = deploy.get('protocol', 'http://')
    PORT = deploy.get('port', '80')
    UI_CERTIFICATE_ENABLED = deploy.getboolean(
        'ui_certificate_enabled', fallback=True)
    ...
    if APPS_ENABLED:
        ...
        for app in apps:
            ...
            config = {
                k: v for (k, v) in app['config'].items() if not k.startswith("ix")}
            chart_id = app['id']
